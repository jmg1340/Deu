...
export default {
  // name: 'ComponentName',
	created() {
		try {
			const arr = this.$q.localStorage.getItem("keyArrNovenaStJosep")
			this.dades.forEach( (dada, index) => dada.fet = arr[index] )		// restablim els checks marcats	
  ...
  data () {
	...
	methods: {
		guardarFet(){
			let arrCheckNovenaStJosep = this.dades.map( (dada) => dada.fet )
			try {
				this.$q.localStorage.set("keyArrNovenaStJosep", arrCheckNovenaStJosep)
			} catch (e) {
		...
		resetAll(){
			this.dades.forEach( (dada) => dada.fet = false )   // posem a false tots els valors
			this.$q.localStorage.remove("keyArrNovenaStJosep")
		}
	}
